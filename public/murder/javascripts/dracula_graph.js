...
Graph.prototype = {
     ...
     *              representation
     */
    addNode: function(id, content, column, row) {
        /* testing if node is already existing in the graph */
        console.log('looking for node ', id)
        if(this.nodes[id] == undefined) {
          console.log('not found, creating')
	        this.nodes[id] = new Graph.Node(id, content, column, row);
 ...
 * Node
 */
Graph.Node = function(id, node, column, row){
    node = node || {};
    node.id = id;
    node.column = column;
    node.row = row;
    console.log("created", id, node, column, row);
    node.edges = [];
    node.hide = function() {
        this.hidden = true;
    ...
    layoutPrepare: function(order) {
        for (i in this.graph.nodes) {
      	...
      	for (i in this.order) {
      	    ...
      	    node.layoutPosX = node.column;
      	    node.layoutPosY = node.row;
      	    counter++;
      	}
    },
